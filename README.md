...
- 统一图片附件：写作和图片模式可以直接上传多张本地图片；视频模式的本地图片仅用于预览，生成视频时需要填写公开图片 URL。
...
- 视频使用 `agnes-video-v2.0`，普通设置提供画面比例、视频长度和公开参考图 URL；本地上传图片不会直接发送给视频 API，高级设置保留帧数、FPS、检查次数和负面提示词。
